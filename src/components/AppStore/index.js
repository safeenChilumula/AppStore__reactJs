...
class AppStore extends Component {
  state = {inputValue: ' ', appStoreList: tabsList[0].tabId}

  AppItem = tabId => {
    const selectApps = tabsList.filter(each => each.tabId === tabId)
    this.setState(prevState => ({
      inputValue: prevState.inputValue,
      appStoreList: selectApps[0].tabId,
    }))
  }
  ...
  searchInput = event => {
    this.setState(prevState => ({
      ...
      appStoreList: prevState.appStoreList,
    }))
  }

  render() {
    const {inputValue, appStoreList} = this.state

    let searchResult = appsList.filter(each => each.category === appStoreList)

    if (inputValue !== ' ') {
      searchResult = appsList.filter(
        each =>
          each.appName.toLowerCase().includes(inputValue.toLowerCase()) &&
          each.category === appStoreList,
      )
    }
        ...
        <ul className="contentContainer">
          {tabsList.map(each => (
            <TabItem
              items={each}
              key={each.tabId}
              relateApp={this.AppItem}
              isActive={appStoreList === each.tabId}
            />
          ))}
        </ul>
